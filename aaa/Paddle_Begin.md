...
-----
  ...
  数据里有些范围特别大，有些又特别小，所以要先进行第一步预处理。范围特别大可能会造成运算时浮点上溢或下溢，范围明显不合理的值也会对模型造成不合理的影响，范围越大影响越大，范围不同也会造成不同属性对模型的重要性不同。另外，有一些机器学习的技巧，如正则化，是基于一些假设的，例如假设所有属性的取值都差不多是以0附近的值（如1、2）为均值且取值范围相近，如果不在这个范围会影响整个训练过程和结果。使用常用的归一化方法，找到
