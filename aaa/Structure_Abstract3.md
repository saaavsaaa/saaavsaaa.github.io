...
```
KMP 通过排除掉很多不必要的对齐位置而快速右移，排除借助了的必要条件（某种模式串自身匹配性），KMP 在第 j 位失配，如果是用 t 取代 j，那么新的前缀 P\[0,t) 必定是原来的 P\[0,j) 的前缀，而原前缀中的 P\[j-t,j)  部分必然也是 P\[0,t) 的前缀（因为原前缀右移了 t 得到的P\[0,t)，说明 j-t 与 0 对齐了，它们相等）,这说明模式串必有一个长度为 t 的前缀与长度为 t 的后缀彼此相等
