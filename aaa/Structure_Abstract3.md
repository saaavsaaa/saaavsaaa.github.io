...
```
此处输入只有一个模式串，它与自己匹配，所以它既是模式串，又是文本串。   

分摊分析：
如果按每个字符去看，模式串中每个字符比较一次，将达到 Ω(m) 次，即使不是每个比较，渐进意义下也是 Ω(m)，而与主串匹配最终将达到 Ω(mn)，但这种分析方法过于粗糙，这是蛮力匹配的程度，最差的情况。但实际上，字符串中的字符组合不可能一点不重，因为字符表有限，而组合无限，何况完全不重很难组合起来表达合适的意义   
为了准确估计，引入一个变量 k，观察 k 在循环中的变化情况：
```
令 k = 2*i - j      //具体含义，详见习题[11-4]
while ( j < m && i < n ) // k 必随迭代而单调递增,故也是迭代步数的上界
  if ( 0 > j || T[i] == P[j] )
    {i++;j++;}  // i、j 同时加1,故 k 恰好加 1
  else 
    j = next[j];  // i 不变，j 至少减 1，故 k 至少加 1
```
next[j] 一定小于 j，所以 k 严格单调递增，所以 k 是迭代步数的上界，只要界定了 k，就确定了算法复杂度的上界：   
k 的初值为 0; 算法结束时，必有（i 至多与 n 同阶，j 至少 是常数，这样 k 最大）: k = 2 * i-j ≤ 2(n-1)-(-1) = 2n - 1，这意味着 k 在渐进意义上不会超过线性范围 O(n)    
next表的构造过程与主算法类似，所以它的复杂度也线性正比于模式串的长度  O(m)    

但是，还有一个问题，例如文本串 00010000.... 模式串00001，当模式串的 0 遇到 文本串的 1 失配后，模式串会去找它短一些的前缀，这个模式串的前缀 00 虽然和文本串的中后缀部分 00 能匹配上，但是 当前匹配字符还是 0 和 1，还是会失配，直到哨兵。但其实由于已经匹配过当前字符，所以 next 表中，当前位置不匹配的前缀其实是可以不用比较的，已经知道了当前字符不是什么了，只需要加一个判断：
```
int * buildNext( char * P ) { //构造模式串P的next[]表
  size_t m = strlen(P)， j = 0; //“主”串指针
  int * N = new int[m]; //next[]表
...
```
蛮力匹配是有比较好的情况的，最好情况线性，每一轮迭代只有常数甚至一次比对，这种好的情况出现的概率，随着字符集规模的增大，概率也会急速地提高，KMP 算法相对于蛮力算法的优势也就不那么明显了。只有在字符集规模相对小时，KMP在性能上的优势才得以展现，所以书上 KMP 的例子多使用二进制字符集。
