...
数组求其中最大的两个值，由最好n-1、最坏2n-3优化，二分递归最差T(n) = 2 * T(n/2) + 2, 左右加2(二分以后各自比较2 * 1)，T(n) + 2 = 2*[T(n/2)+2] = 2²*[T(n/4)+2] = ... = 2<sup>k</sup> * [T(n/k)+2],k=logn。当数组只有两个元素T(2) = 1，只有三个元素T(3) <= 3。递归基最差情况T(3) = 3，n/2<sup>k</sup> = 3，2<sup>k</sup> = n/3，T(n) + 2 = 2<sup>k</sup> * [T(n/k)+2] = n/3 * (T(3)+2)，T(n)=5n/3-2   
