...
可进一步优化，比如：
1.仿照 uniquify( )高效版的思路，元素移动的次数可降至0(n)，但比较次数依然是0(n2) ;而且,稳定性将被破坏     
2.先对需删除的重复元素做标记,然后再统一删除。稳定性保持,但因查找长度更长,从而导致更多的比对操作     
3. V.sort().uniquify() :简明实现最优的O(nlogn)     
没看uniquify实现，比较好的方法我觉得应该是，先排序，然后用一个全局变量记录需要前移的值，每删一个元素，将需要前移的值+1，当遇到待移动值，按照当前记录的需要前移的值移动，最终更新数组长度     
