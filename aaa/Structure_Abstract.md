...
求解：T(n) - n = T(n-1) + O(1) - n = T(n-1) - (n-1) = T(n-2) - (n-2) = ... = T(2) - 2 = T(1) - 1 = T(0) - 0     
      T(n) - n = T(0) - 0 = O(1) - 0       
      T(n) = O(1) + n =  O(n)   
...
一分为二和递归都很简单，核心在于合并     
二路归并 2-way merge :将两个有序序列合并为-一个有序序列: S\[lo, hi) = S\[1o, mi) + S\[mi, hi)      
由于归并过程，每个原序列被替换的位置的元素或者就是当前进行正进行比较的元素，或是在子序列中还没比较到的元素，所以不担心被覆盖，也就不用新开闭空间保存子序列，画图看更直观     
一分为二之后，归并时只需判断前半段是否全比较完了，因为前半段如果先比较完，那么后半段中未比较的元素可以原地不动了     
算法的运行时间主要消耗于for循环,共有两个控制变量:初始:j = 0,k= 0;最终:j = lb,k= lc;亦即:j+k=lb+lc=hi-lo=n     
观察:每经过一次迭代，j和k中至少有一个会加一(j + k也必至少加一)     
故知: merge()总体迭代不过O(n)次，累计只需线性时间!     
这一结论与排序算法的Ω(nlogn)下界并不矛盾---毕竟这里的前半段和后半段均已各自有序     
总迭代次数与每次迭代中的二分归并合一起，最差的复杂度至少O(nlogn)     
注意:待归并子序列不必等长     
亦即:即允许lb≠1c,mi≠(1o+hi)/2     
...
v[i]的物理地址 = v + i x s，s为单个单元占用的空间量   

既然同属线性序列，列表固然也可通过秩来定位节点∶从头/尾端出发，沿后继/前驱引用...   
然而，此时的循秩访问成本过高，已不合时宜 //List::operator[](Rank r)，下节详解兼顾两种访问方式的 **skiplist，第九章**   
因此，应改用循位置访问( call-by-position )的方式,亦即，应转而利用节点之间的相互引用，找到特定的节点      
...
选择排序的无序部分是前缀部分，并且前缀部分的值不会大于有序部分(后缀)的第一个值     
插入排序的无序部分是后缀，值的大小和有序部分没关系，有序部分就是原序列的前k个元素     

最好情况∶完全（或几乎）有序，每次迭代，只需1次比较，0次交换，累计O(n)时间!     
最坏情况∶完全（或几乎）逆序第k次迭代，需O(k)次比较，1次交换累计O(n²)时间!     

假定∶各元素的取值遵守均匀、独立分布∶     于是:平均要做多少次元素比较?     
考查:L[r]刚插入完成的那一时刻:     试问∶此时的有序前缀L[0，r]中，哪个元素是此前的L[r]?
观察∶其中的r + 1资元素均有可能，且概率均等于1/(r + 1)       
因此，在刚完成的这次迭代中，为引入S[r]所花费时间的数学期望为 [r + (r - 1) + ...+ 3 + 2 + 1 + 0]/(r + 1) + 1 = r/2 + 1，  (加权平均，+1是因为0次交换虽然不需要交换但需要比较1次)     
于是，总体时间的数学期望 = [0 + 1 +...+ (n - 1)]/2 + 1 = O(n²)，数学期望的线性率（总和的期望等于各自期望的总和，无论是否相关）   
平均性能O(n²)与最差同阶，这说明虽然最好情况O(n)，但它出现概率极低     

逆序对inversion：任意两个逆序元素组成一个逆序对(不要求相邻)，那么有序和无序部分交界处的无序元素需要向前移动的长度就是有序部分元素与它组成的逆序对个数，于是复杂度就是移动次数I和插入次数n(每个元素一次):O(I + n)。最好情况是I=0，所O(n)；最差情况，任意两个元素都逆序所以C(n,2)，与O(n²)同阶。算法的复杂度取决于逆序对I的情况，这种算法叫输入敏感性算法input-sensitive，后面讲shellsort希尔排序，这种特性对希尔排序的性能和有效性至关重要。
