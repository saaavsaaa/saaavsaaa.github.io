...
#### 4.2.6 GMM模型的迭代   
--tot-gauss: 在 GMM-HMM 语音 系统 中 ， 每个 HMM 状态 中 包含 一个 独立 的 GMM 概率 分布 模型 ， 该 参数 指定 整个 语音 系统 中 所有 的 GMM 合 到 一起 的 高斯 总数 。 在 更早 的 一些 语言 工具箱 中 ， 每个 HMM 状态 中的 高斯混合 数量是 一致 的 ， 比如 每个 状态 32高斯、 64 高斯 ， 但 在 Kaldi 中 并没有 这样 地 强制 限制 ， 数据 量 充足 的 状态 更为 重要 ， 因此 也 会 获得 更多 的 高斯 配额 ， 但 整个 系统 的 总 高斯 数 还是 要 限定 在 可控 范围 内 。    
