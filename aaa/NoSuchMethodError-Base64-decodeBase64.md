...
```
由于不懂C++,就全贴这了，不过大概可以看的出，是根据索引（例如：#33）去常量池读取方法字节码，然后执行，往下就只能等我再学学C++再说了。执行到invokestatic指令时，类如果没有初始化，则会初始化类，需要确认下是否初始化了，但是初始化了错误的类，写好了btrace脚本，等星期一运维来了运行一下看看？？？？？？？？？？。
先回头看看类加载吧。
StackMapTable，无论是调用处，正常和不正常的被调用处都没有，书上说没有就是相当于有个等于0的，似乎和这问题也没什么关系。
javac编译时没有连接步骤，是在虚拟机加载Class文件时进行动态连接。虚拟机运行时，从常量池获得对应符号引用，再在类创建时运行解析、翻译到具体内存地址。这问题是不是翻译完了，结果找不到实例，或者找错了，或者加载不到？
