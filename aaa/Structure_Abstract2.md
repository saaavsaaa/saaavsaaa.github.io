...
```
log(n)，可以优化它的常数操作，每一次比较交换都是3次操作，比如先备份待插入的词条，而是与上级比较，比较后如果需要交换，直接将上级复制到最后，并继续比较和将上级覆盖到它当前的位置，直到找到不破坏堆序性的位置，才将它盖到这个位置。就可以从3log(n)优化到log(n)+2。比较操作也可以优化，参见配套习题解析。   
