...
**BB-3?** :s为红（其孩子均为黑）。zag(p)或zig(p);红s转黑，黑 p 转红黑高度依然异常，但 r 有了一个新的黑兄弟 s' 故转化为前述情况，而且既然p已转红，接下来绝不会是情况BB-2B而只能是BB-1或BB-2R。于是，再经一轮调整之后红黑树性质必然全局恢复。相当于，平衡三个分支的黑高度，使三个分支黑高度相同，并将下溢向上传导   
```
      P_b                      S_b
  S_r        X_b          0           P_r
0    1(s')   2(R_b)               1(s')   X_b
