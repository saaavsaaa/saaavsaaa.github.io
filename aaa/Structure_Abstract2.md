...
(u² + v²)(s² + t²) = (us+vt)²+(ut-vs)²   
(2² + 3²)(5² + 8²) = (10+24)²+(16-15)²   
就不难推知  :任一自然数 n 可表示为一对整数的平方和，当且仅当在其素分解中，形如 M = 4k + 3 的每一素因子均为偶数次方(因为奇数次方不是平方项)   
正向子序列和逆向子序列中有值冲突，设a、b冲突 1 ≤ a，b < M/2，就意味着 -a² ≡ b²(mod M)，a² + b² = n 可以整除 M，所以表长 M 一定是 n 的素因子，如果 M = 4k + 3，那 n 也可以被 M²(偶数次方) 整除，那么 n 最小也得是 M²，M² ≤ a² + b² 这于 a，b < M/2 (M²/4 + M²/4) 冲突，所以 M = 4k + 3 的正逆两个子序列中不会有冲突   

桶排序/计数排序：复杂度O(n+M)或O(n与M中的大者=max(n,M))     
对 cdebcce 排序(对英文字符排序桶个数 M 最大不过26)，排序结果：   
|key/value|a|b|c|d|e|...|O(n+M)字符排序后的结果|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:--|
|accum[] (积分/累加)|0|1|4|5|7|7...|O(M)当前字符及小于当前字符的所有字符个数的累加|
|count[] (个数)|0|1|3|1|2|0...|O(n)当前字符的个数，最大26|

c有三个，其中最后一个c在第4位(包括它共有4个字符)，而前一个字符b的最后一个在第1位，所以排序后的字符串的第2、3、4位是c   

### 优先级队列
例如医院，普通病人就是先来先看，但突然来了个急诊心脏病人，优先级就比较高，要先看   
在 **Huffman编码树** 的构造算法中，需要将整个森林组织为某种高效的数据结构,从而可以使得我们可以反复地从中取出权重最小的两棵子树,并将它们合并之后,重新加入到森林之中。   
...
template <typename T> struct PQ { //priority queue
  ...
  virtual T de1Max()]= e; //删除优先级最高的词条
}; //与其说PQ是数据结构，不如说是ADT;其不同的实现方式，效率及适用场合也各不相同
```
pure virtual function 纯虚函数，强制要求之后必须提供实现   
Stack和Queue ,可以看做是PQ的特例，优先级完全取决于元素的插入次序，或者说优先级都一样   

如果用向量实现，取最高优先级时需遍历，删除同样有向量的缺点，所以不合适   
列表类似
有序向量，优先级最高的向量一直在最后所以O(1)，但插入的复杂度过高，要维护优先级   
有序列表也不行，插入高   
BBST:AVL、Splay、 Red-black :实现这三个接口均只需O(1ogn)时间，getMax还可以优化到O(1)，但是, BBST的功能远远超出了PQ的需求，查找和删除都只针对最大节点，其他节点也具有的高效操作这里用不上   
若只需查找极值元，则不必维护所有元素之间的全序关系,偏序足矣，而不用像BBST一样维护全序关系   
因此有理由相信，存在某种更为简单、维护成本更低的实现方式，使得各功能接口，时间复杂度依然为O(1ogn),而且实际效率更高  

利用类似完全二叉树 Complete Binary Tree的结构，它相当于是平衡因子处处非负(0、1)的AVL，要么是满树，要么有左子树比右子树高一个节点的情况   

逻辑上，等同于完全二叉树   
物理上，直接借助向量实现   
逻辑节点与物理元素，依层次遍历次序彼此对应   
向量的秩与完全二叉树层次遍历的次序一一对应，注意：并不是与节点值一一对应   
...
0
 12
   3456
```
在满树的部分，由于后一行的元素个数一定前一行的两倍，第n行的第一个元素次序一定是2ⁿ-1（n从0开始），而它前面一定有2ⁿ-1个元素，n-1行的第一个元素的次序是（2ⁿ÷2)-1。于是可知，满树的一个节点的次序是i，如果存在，那么它的左子节点的次序是2i+1，右子几点的次序是2(i+1)。而因为是完全二叉树，所以不满的节点一定是尾部的，所以不需要担心：   
...
```
堆序性，子节点的次序一定小于上级节点，于是最大元在根处，返回最大元素，只需返回根即可   

插入、上滤：   
为插入词条e,只需将e作为末元素接入向量(不是选节点挂到后面，而是直接放在后面)，不会破坏结构，然后按照规律找到它的上级节点，比较后如果发现若堆序性也亦未破坏，则完成；如果破坏了，就交换它和上级节点，然后继续比较它和新的上级，最坏情况持续到根。   
```
...
return i: //返回 上滤最终抵达的位置
```
log(n)，可以优化它的常数操作，每一次比较交换都是3次操作，比如先备份待插入的词条，而是与上级比较，比较后如果需要交换，直接将上级复制到待插入的词条原本的位置，并继续比较和将上级覆盖到它当前的位置，直到找到不破坏堆序性的位置，才将它盖到这个位置。就可以从3log(n)优化到log(n)+2。比较操作也可以优化，参见配套习题解析。实际很少出现最坏情况，更加精细的估算表明，在通常的随机分布下，每个节点上升的平均高度实际上只不过是常数，此结论也可参考习题解析。   
上级节点的秩，二分之一节点个数的下整在减一   

删除与下滤：   
最大元出队后，需要删除，删除了根后，为了保证结构，可以先将最后元素放到树根位置，但是这样可能会导致堆序被破坏，交换它与两个子节点中大的那个进行交换，因为在最后一层时，它可能大于其中一个子节点，但小于另一个，所以要与子节点中大的那个进行交换(因为偏序)   
最大元素始终在堆顶,故为删除之,只需摘除向量首元素，代之以末元素e(结构性保持;若雄序性依然保持则完成)否则e与孩子中的大者换位(若堆序性因此恢复,则完成)否则e再次与孩子中的大者换位，这个过程称为下滤   
```
...
即便只考虑底层,n/2 个叶节点,深度均为O(logn),累计耗时O(nlogn),而这个时间完全可以做到全序，事实上只需要偏序    

自下而上的下滤:
任意给定堆 H₀ 和 H₁ ,以及节点 p , 为得到堆 H₀ U {p} U H₁ ,只需将 r₀ 和 r₁，当作 P 的孩子，对 p 下滤，类似删除最大元的操作（当前无序），等效于逐层合并规模增大   
```
template <typename T>
...
每个内部节点所需的调整时间，正比于其高度而非深度，整个算法复杂度对应每个节点下降的是高度的总和，经过推算不难得知，这个总和在渐进意义上无非是O(n)，推导过程可参考习题解析   
因为树节点越往下个数越多，所以高度总和远小于深度总和，底层节点个数最多，底层节点高度比较小而深度比较大   

包括完全二叉维在内的任何一种优先级队列都天生地具有选取功能，选取最大元   
数组排序之前学过的选择排序O(n²)，selection sort每选当前最大元素要遍历一次   
...
循环 m 次，每次合并 log(n + m)
O(m * ( logm + log(n + m))) = o( m * log(n + m) )

方法二，Floyd算法(也就是自下而上的下滤): union( A,B ).heapify( n + m）  O( m + n )

```
Foyd算法的输入在默认情况下，所有的元素都是无序排列的。没有利用到A和B的偏序关系。   
左式堆可以高效合并堆:C. A. Crane,1972:保持堆序性，附加**新条件**，使得在堆合并过程中，只需调整很少部分的节点 O(logn)   
新条件 = 单侧倾斜：节点分布偏向于左侧；合并操作只涉及右侧（发明者的惯例）   
右侧链长度控制在大O意义下的 log(n) 以内   
左式堆的堆序性可以保证，但结构性不能保证   
不过实际上，结构性并非堆结构的本质要求   

为了度量和判断左式堆的单侧倾斜性,我们需要引入一个概念,也就是所谓的“空节点路径长度”( NPL )。   
为了便于理解，用在红黑和B树中的技巧，引入足够多的外部节点，使树成为真二叉树（度数为偶数）   
Null Path Length :   
 0\) npl(NULL) = 0   (外部节点)   
 1\) npl(x) = 1 + min( npl( lc(x) ), npl( rc(x) ) )左右子节点中较小的加1   
把这个min()换成max()，也就是不折不扣的高度。   
验证：   
npl(x) = x到外部节点的最近距离   
npl(x) = 以x为根的最大满子树的高度   
左倾：上对任何内节点 x，都有npl( lc(x) ) ≥ npl( rc(x) )，推论∶对任何内节点 × ，都有npl(x) = 1 + npl( rc(x) )   
满足左倾性 leftist property 的堆，即是左式堆。左倾性与堆序性，相容而不矛后，左式堆的子堆必是左堆，左式堆倾向于更多节点分布于左侧分支   
但这只是趋势，左不小于右，不一定严格大于右   
rchain(x) 右侧链: 从节点x出发，一直沿右分支前进   
特别地，rChain(root)的终点 , 必为全堆中最浅的外部节点 npl(r) = | rchain(r) | = d   
存在一棵以r为根、高度为d的满子树    
右侧链长为 d 的左式堆，至少包含 2ᵈ - 1 个内部节点，2<sup>d+1</sup> - 1个节点   
反过来如果将左式堆的规模固定为 n ,那么右侧链的长度 d 也就至多不过 log(n), 右侧链的长度 d ≤ log₂(n + 1) - 1 = O(1ogn)   

合并：
```
template <typename T> //基于二叉树，以左式堆形式实现的优先级队列
class PQ_LeftHeap: public PQ<T>， public BinTree<T> { //由于不再满足结构性，就不能再用向量了，直接用二叉树
public:
...
void PQ_LeftHeap<T>::insert( T e ) {  // O(logn)
  BinNodePosi(T) v = new BinNode<T>( e ); //为e创建一个二叉树节点           插入单个节点就相当于插入一个只有根的左式堆
  _root = merge( _root，v ); // 通过合并完成新节点的插入
  _root->parent = NULL; // 既然此时堆非空，还需相应设置父子链接
  size++; //更新规模
}
```
删除：将根摘除（优先级队列出队），并将原来的左右子堆合并
```
