...
2^k去摸计算虽然快，但冲突的概率相对也更大，对 M 同余的所有数会被映射到同一个位置。最大公约数为1的可能冲突概率最小，数据在映射范围内分布的也最均匀（根据数论知识），所以最好选择质数，它的因数最少      
实际中处理数据多有一种局部性 locality，其中一种典型是以某一步长 step 在序列中单调分布(程序中while等循环处理的)，S步长、M序列长度、G公因子： gcd (S , M) = G
