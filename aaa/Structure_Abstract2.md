...
L和R或者不存在，或者所含的关键码均不足[m/2]个。注意，L和R仍必有其一，且恰含_[m/2]-1 个关键码（不妨以为例)。那发生下溢的当前超级节点和(L或R)的关键码加起来恰好是 m-2 个关键码、m-1 个分支。此时，可以将它们合并，为了保证中序的顺序性，将它们的直接前驱作为交接点，连接这两个超级节点。这样，就刚好m-1 个关键码、m 个分支。此时，直接前驱超级节点本身也可能发生下溢，解决方法同上。次数最多树高，复杂度O(h)    
