...
在满树的部分，由于后一行的元素个数一定前一行的两倍，第n行的第一个元素次序一定是2ⁿ-1（n从0开始），而它前面一定有2ⁿ-1个元素，n-1行的第一个元素的次序是（2ⁿ÷2)-1。于是可知，满树的一个节点的次序是i，如果存在，那么它的左子节点的次序是2i+1，右子几点的次序是2i+2。而因为是完全二叉树，所以不满的节点一定是尾部的，所以不需要担心：   
