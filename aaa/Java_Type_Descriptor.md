...
方法的 ASM 树 API 是基于 MethodNode的。instructions 字段是一个指令列表,InsnList类型。InsnList 是一个由指令组成的双向链表, AbstractInsnNode 中有两个属性AbstractInsnNode prev和AbstractInsnNode next保持指向的引用。
