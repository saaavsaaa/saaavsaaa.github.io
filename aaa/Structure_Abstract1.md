...
简单来说，先讲了用列表递归表示，就像数据表里用一张表表示多个层级的形式，下一级中有一个字段指向上一级的ID，但这种形式寻找子节点要遍历全集O(n)。进一步可以在包含子节点的节点上用指针指向所有子节点，美中不足是各个节点的子节点数量可能并不平均，子节点个数和子树根节点的出度同阶，只有平均了之后才能到O(1)，另外极端情况下可能子节点的长度达到n，比如所有节点都在根下。   
